...
class WriteTarget{
    ...
    set outputSource(outputSource){
        ...
        if(!path.isAbsolute(outputSource)){
            return this._outputSource = path.normalize(outputSource);
        }
    ...
    async write(){
        if(!this.buffer) this.buffer = await this.bufferPromise;
        if(!this._outputPath){
        ...
        await writer.writePdfToFs(this._outputPath, this.buffer);
        this._writeCount++;
...
module.exports = (servePath, eleventyConfig) => {
    ...
    // return wrapper api
    return {
        addWriteTarget: (outputSource, htmlContentOrUrl, pdfOptions) => new WriteTarget(outputSource, htmlContentOrUrl, pdfOptions), 
        collate,
        getServerInfo(){
