...
class PdfWriter{
     ...
     * @param {boolean} [headless=true] when false, puppeteer window will be visible to user; defaults to true
     * may be set after construction via setter.
     */
    constructor(servePath, headless=true){
    ...
    async getPdfBuffer(htmlContentOrUrl, pdfOptions, isRelUrl=false){
        ...
        if(isRelUrl){
            ...
            await page.goto(`http://localhost:${this._servePort}${path.beginsWith('/') ? '' : '/'}${path}`, {
