...
class PdfWriter{
    ...
    async getPdfBuffer(htmlContentOrUrl, pdfOptions, isRelUrl=false){
        ...
        if(isRelUrl){
            let relPath = htmlContentOrUrl, 
                fullPath = `http://localhost:${this._servePort}${this.pathPrefix}${relPath.startsWith('/') ? relPath.replace('/', '') : relPath}`;
...
function getWriter(servePath, eleventyConfig){
